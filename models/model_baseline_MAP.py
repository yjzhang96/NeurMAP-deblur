...
class DeblurNet():
    ...
    def update_M(self,warmup=False):
        ...
        ## offset to 0/1 loss
        self.loss_d_sharp = self.L1loss(mmap_real_S_norm, zero_tensor)  # shape motion -> 0
        mmap_real_B_norm_detach = mmap_real_B_norm.detach()
        self.loss_d_fake_sharp = self.L1loss(mmap_fake_S_norm, mmap_real_B_norm_detach)  # deblur motion -> blur motion
        ## motion map B as large as possible (upper limit)
        self.loss_d_blur = self.L1loss(torch.abs(mmap_real_B_norm), one_tensor)    # blur motion -> 1
    ...
    def test(self, validation = False):
        with torch.no_grad():
            ...
            self.fake_B, _ = self.blur_net(self.real_S, mmap_B_S)  # reblurring from original sharp image

            mmap_fake_S = self.net_M(self.fake_S)
            mmap_B_fS = self.vec_diff(mmap_real_B,mmap_fake_S)
            self.fake_B_from_fS, _ = self.blur_net(self.fake_S, mmap_B_fS) # reblurring from fake sharp image
    ...
    def save(self,epoch):
        ...
        save_globald_filename = 'naturalD_net_%s.pth'%epoch
        ...
        if len(self.config['gpu'])>1:
            ...
            torch.save(self.net_naturalD.module.state_dict(),os.path.join(self.config['checkpoints'], self.config['model_name'],save_globald_filename))
            ...
            torch.save(self.net_naturalD.state_dict(),os.path.join(self.config['checkpoints'], self.config['model_name'],save_globald_filename))
