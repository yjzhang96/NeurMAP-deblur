...
class DeblurNet():
    ...
    def update_D(self,warmup=False):
        ...
        if self.real_S_exist:
            img_diff = torch.mean(torch.abs(self.real_B - self.real_S),dim=1,keepdim=True)
            mask = torch.where(img_diff>0.1, one_tensor, zero_tensor)
        ...
        self.loss_d_blur = self.L1loss(torch.abs(mmap_real_B_norm), mask)
    ...
    def update_G(self,warmup=False):
        ...
        # self.G_tv_loss = self.L1loss(self.fake_S[B//2:,:,:,:-1],self.fake_S[B//2:,:,:,1:]) + \
        ...
        loss_G =  lambda_G_content * self.loss_content + self.loss_adv_G +  lambda_G_tv * self.G_tv_loss \
    ...
    def get_current_visuals(self):
        ...
        if self.config['is_training']:
            mask = utils.tensor2im(self.mask)
            return OrderedDict([('real_B',real_B),('real_S',real_S),('fake_S',fake_S),('fake_B',fake_B),('mask',mask)])
