...
class DeblurNet():
    ...
    def update_D(self,warmup=False):
        ...
        # lambda_tv = 0.002
        self.tv_loss = self.L1loss(offsets[B//2:,:,:,:,:-1],offsets[B//2:,:,:,:,1:]) + \
                        self.L1loss(offsets[B//2:,:,:,:-1,:],offsets[B//2:,:,:,1:,:])
    ...
    def update_globalD(self):
        ...
        # import ipdb; ipdb.set_trace()
        pred_real = self.net_globalD(real_batch)
    ...
    def update_G(self,warmup=False):
        ...
        bmap_fake_S_abs = self.vec_norm(bmap_fake_S)
        zero_tensor = torch.zeros_like(bmap_real_B).cuda()
        self.loss_adv_G_fS =  self.L1loss(torch.abs(bmap_fake_S_abs[B//2:]), zero_tensor[B//2:])
        ## relativistic loss
        ...
        ## G tv loss
        self.G_tv_loss = self.L1loss(bmap_fake_S_abs[B//2:,:,:,:-1],bmap_fake_S_abs[B//2:,:,:,1:]) + \
                        self.L1loss(bmap_fake_S_abs[B//2:,:,:-1,:],bmap_fake_S_abs[B//2:,:,1:,:])
        ...
        if self.config['train']['relative_reblur']:
            # import ipdb; ipdb.set_trace()
            bmap_real_B_abs = self.vec_norm(bmap_real_B)
            reblur_bmap = bmap_real_B_abs[half_B:] - bmap_fake_S_abs[half_B:]
        elif self.config['train']['absolute_reblur']:
            
            reblur_bmap = bmap_real_B
        reblur_bmap_detach = reblur_bmap.detach()
        self.fake_B, _ = self.blur_net(self.fake_S[half_B:], reblur_bmap_detach)
    ...
    def get_tensorboard_images(self):
        ...
        return paired_train_img, unpaired_train_img, bmap_vis
